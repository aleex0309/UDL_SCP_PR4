...
public class SimulationAP implements Simulation {
    ...
    public void startSimulation() throws SimulationException {
        ...
        try {
            ...
            for(int id = 0; id < SimulationProperties.getNumberOfThreads(); id++) { //Thread creation
                Threads[id] = new SimulationLogicAP.ThreadCalculator(id);
                Threads[id].start();
            }
            ...
            for (long i = 0; properties.isInfiniteSimulation() || i < properties.getNumberOfIterations(); i++) {
                try {
                    ...
                    ThreadEndSemaphore.acquire(SimulationProperties.getNumberOfThreads()); // Wait for all threads to finish
