...
public class SimulationLogicAP implements SimulationLogic {
    ...
    public static class ThreadCalculator extends Thread{
        private int id;
        ...
        public void run(){
            ...
            for(int iteration = 1; iteration <= SimulationProperties.getNumberOfIterations(); iteration++){
                ...
                //Calculamos rangos para cada thread
                int particlesPerThread = simulation.getObjects().size()/SimulationProperties.getNumberOfThreads();
                int residue = simulation.getObjects().size()%SimulationProperties.getNumberOfThreads();

                int start;
                int end;
                if(id < residue){ //Asignamos el resto de particulas a los primeros hilos
                    start = id*(particlesPerThread+1);
                    end = start + particlesPerThread + 1;
                }else if (id == residue){
                    start = id*(particlesPerThread+1);
                    end = start + particlesPerThread;
                }else{
                    start = id*particlesPerThread + residue;
                    end = start + particlesPerThread;
                }
                //System.out.println("Thread " + id + " start: " + start + " end: " + end);
    ...
    public void calculateAllNewValues() {
        //Doesn't do anything, just a reference to the main thread
    }
    ...
    private static void bounceFromScreenBorders(SimulationObject newObject) {
    ...
    private static void moveObject(SimulationObject newObject) {
    ...
    private static TripleNumber calculateVelocity(ImmutableSimulationObject object) {
